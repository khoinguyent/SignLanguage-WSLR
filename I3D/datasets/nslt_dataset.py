...
def make_dataset(split_file, split, root, num_classes, rate = 1):
    dataset = []
...
class NSLT_Test(data_utl.Dataset):

    def __init__(self, split_file, split, root, transforms=None, rate=1):
        self.num_classes = get_num_class(split_file)
    ...
    def __getitem__(self, index):
        ...
        rgb_imgs = vp.load_rgb_frames_from_video(self.root['word'], vid, start_f, total_frames)
        flow_imgs = vp.load_flow_frames_upd(self.root['word'], vid, start_f, total_frames, self.rate)

        #print(vid, imgs.shape)
        rgb_imgs, label = self.pad(rgb_imgs, label, total_frames)
        flow_imgs, label = self.pad(flow_imgs, label, total_frames)

        rgb_imgs = self.transforms(rgb_imgs)
        flow_imgs = self.transforms(flow_imgs)

        ret_lab = torch.from_numpy(label)
        ret_rgb_img = vp.video_to_tensor(rgb_imgs)
        ret_flow_img = vp.video_to_tensor(flow_imgs)

        return ret_rgb_img, ret_flow_img, ret_lab, vid
